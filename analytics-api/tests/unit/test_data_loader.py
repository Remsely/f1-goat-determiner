...
class TestFakeDataLoaderSeasonFiltering:
    ...
    def test_races_without_seasons_returns_all(self, loader) -> None:
        df = loader.races()
        assert len(df) == 6

    def test_races_with_single_season(self, loader) -> None:
        df = loader.races(seasons=[2024])
        assert len(df) == 3
        assert (df["year"] == 2024).all()

    def test_races_with_multiple_seasons(self, loader) -> None:
        df = loader.races(seasons=[2023, 2024])
        assert len(df) == 6

    def test_races_with_nonexistent_season(self, loader) -> None:
        df = loader.races(seasons=[1950])
        assert len(df) == 0

    def test_results_filtered_by_season(self, loader) -> None:
