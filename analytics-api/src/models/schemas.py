...
class DriverStats(BaseModel):
    ...
    poles: int = Field(description="Поул-позиции", example=68)
    ...
    pole_rate: float = Field(description="Процент поулов", example=22.08)
    title_rate: float = Field(description="Процент титулов от сезонов", example=36.84)
    avg_championship_pct: float = Field(
        description="Средняя позиция в чемпионате (0-100%, где 100% = 1 место)", example=84.26
    )
...
class Tier(BaseModel):
    ...
    avg_pole_rate: float = Field(description="Средний процент поулов", example=15.3)
    avg_finish: float = Field(description="Средняя финишная позиция", example=4.5)
...
class TierListMeta(BaseModel):
    ...
    seasons: list[int] | None = Field(
        description="Фильтр по сезонам (null = все)", example=[2020, 2021, 2022, 2023, 2024]
    )
    ...
    silhouette_score: float = Field(
        description="Коэффициент силуэта (качество кластеризации, от -1 до 1)", example=0.339
    )
