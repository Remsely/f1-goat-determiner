...
export const DriverModal = ({ driver, isOpen, onClose }: DriverModalProps) => {
  ...
  const winsPerPodium = stats.podiums > 0 ? ((stats.wins / stats.podiums) * 100).toFixed(1) : '0';
      ...
      <div className={styles.content}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.statsGrid}>
            ...
            <StatRow label="Avg.  Championship Position" value={`${stats.avg_championship_pct.toFixed(1)}%`} />
        ...
        {(stats.titles > 0 || stats.podiums > 0) && (
          <div className={styles.section}>
            ...
            <div className={styles.statsGrid}>
              {stats.titles > 0 && <StatRow label="Title Rate" value={`${stats.title_rate.toFixed(1)}%`} />}
              {stats.podiums > 0 && <StatRow label="Win/Podium Conversion" value={`${winsPerPodium}%`} />}
            </div>
          </div>
        )}
      </div>
